...
ENV VIRTUAL_ENV /opt/pydantic-all-in-one-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/pydantic-all-in-one/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/pydantic-all-in-one/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/pyd4all/ && touch src/pyd4all/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/pydantic-all-in-one/
